...
(function() {
	...
	function selectTab(group, option, focusIndex) {
		if (DEBUG_MODE) console.log(`Group ${group} option ${option} selected with focus ${focusIndex}`);
		...
		if (focusIndex !== void 0) buttons[focusIndex].el.focus();
	...
	function handleTabKeyDown(event, group, buttonIndex) {
		...
		function normalizeIndex(index) {
			if (index < 0) return index + buttons.length;
			if (index >= buttons.length) return index - buttons.length;
			return index;
		}
		let newIndex;
		switch (event.key) {
		...
		if (newIndex !== void 0) {
			if (DEBUG_MODE) console.log(`Keypress ${event.key} leads to new button index ${newIndex}`);
	...
	function getGroup(el) {
		return el.dataset.group || el.closest("[data-group]").dataset.group || void 0;
	}
	function initTabs() {
		let urlHash = window.location.hash;
		if (urlHash) urlHash = decodeURIComponent(urlHash.slice(1)).replace(/ /g, "_");
		document.querySelectorAll(".tab-group-container").forEach((container) => {
			if (!container.dataset.group) container.dataset.group = randomGroup();
		});
		document.querySelectorAll(".tab-button-container, .tab-panel-container").forEach((container) => {
			container.querySelectorAll(":scope > .tab-button, :scope > .tab-panel").forEach((elem, index) => {
				if (!elem.dataset.option) elem.dataset.option = index;
			});
		});
		...
		document.querySelectorAll(".tab-button").forEach((button) => {
			...
			if (!option) return;
			if (!allButtons[group]) allButtons[group] = [];
			const buttonIndex = allButtons[group].length;
			allButtons[group].push({
				option,
				el: button
			});
			if (!defaultButtons[group]) defaultButtons[group] = option;
			if (urlHash && name.replace(/ /g, "_") === urlHash) defaultButtons[group] = option;
			button.role = "tab";
			button.tabIndex = 0;
		...
		document.querySelectorAll(".tab-panel").forEach((panel) => {
			...
			panel.role = "tabpanel";
			panel.tabIndex = 0;
			if (!allPanels[group]) allPanels[group] = [];
			allPanels[group].push({
				option,
				el: panel
			});
		});
		for (const [group, option] of Object.entries(defaultButtons)) selectTab(group, option);
		if (DEBUG_MODE) {
