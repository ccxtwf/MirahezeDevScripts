//#region gadgets/PowertoolsPlacement/code.js
(function($, mw) {
	...
	if (mw.libs.PowertoolsPlacement !== void 0 && mw.libs.PowertoolsPlacement.versionString >= versionString) return;
	if ($(POWER_EDITOR_TOOLBOX_PORTLET_ID).length > 0) return;
	var i18n = {
		"ab": "Мои инструменты",
		"ace": "Alat saya",
		"af": "My hulpmiddels",
	...
	*/
	function placeAfterNode(insert, atNode) {
		if (atNode.length === 0) return null;
		atNode.after(insert);
		return insert;
	}
	/**
	...
	*/
	function addPowerEditorToolboxPortlet(skin) {
		if ($("#" + POWER_EDITOR_TOOLBOX_PORTLET_ID).length) return $("#" + POWER_EDITOR_TOOLBOX_PORTLET_ID).first();
		var portlet = null;
		switch (skin) {
			case "vector":
			case "vector-2022":
	...
	*/
	function createPowerEditorToolboxPortletForGamepress() {
		var portlet = $("<div>", {
			...
			"id": POWER_EDITOR_TOOLBOX_PORTLET_ID,
			"role": "navigation"
		}).append($("<h3>", { "class": "widget-title" }).text(POWER_EDITOR_TOOLBOX_PORTLET_HEADING_TEXT), $("<ul>"));
	...
	*/
	function createPowerEditorToolboxPortletForMedik() {
		var newMenu = $("<div>", { "class": "dropdown" }).append($("<a>", {
			...
			"aria-haspopup": "true",
			"aria-expanded": "false"
		}).text(POWER_EDITOR_TOOLBOX_PORTLET_HEADING_TEXT), $("<div>", { "class": "dropdown-menu dropdown-menu-end" }).append($("<div>", {
			"role": "navigation",
			"id": POWER_EDITOR_TOOLBOX_PORTLET_ID,
			...
			href: "#",
			role: "button"
		}).text(POWER_EDITOR_TOOLBOX_PORTLET_HEADING_TEXT), $("<div>", { "class": "mw-portlet-body" }).append($("<ul>", { lang: lang || "en" })))));
	...
	*/
	function createPowerEditorToolboxPortletForCitizen() {
		var newMenu = $("<div>", { "class": "citizen-header__item citizen-dropdown" }).append($("<details>", {
		...
		}).append($("<span>", { "class": "citizen-ui-icon mw-ui-icon-edit mw-ui-icon-wikimedia-edit" }), $("<span>").text(POWER_EDITOR_TOOLBOX_PORTLET_HEADING_TEXT))), $("<div>", {
			id: "citizen-powertools-portlet-container__card",
			"class": "citizen-menu__card"
		}).append($("<div>", { "class": "citizen-menu__card-content" }).append($("<div>", {
		...
		}).append($("<div>", { "class": "citizen-menu__heading" }).text(POWER_EDITOR_TOOLBOX_PORTLET_HEADING_TEXT), $("<div>", { "class": "citizen-menu__content" }).append($("<ul>", { "class": "citizen-menu__content-list" })))))));
		$(".citizen-header__start").append(newMenu);
		$("body").on("click", function() {
			if ($("#citizen-powertools-portlet-container-details").attr("open")) $("#citizen-powertools-portlet-container-details").attr("open", null);
		});
		return newMenu.find("#" + POWER_EDITOR_TOOLBOX_PORTLET_ID);
	}
	/**
	...
	*/
	function createFloatingMenuPowerEditorToolboxPortlet() {
		var portlet = $("<div>", {
			id: POWER_EDITOR_TOOLBOX_PORTLET_ID,
			"class": "floating"
		}).append($("<div>").append($("<div>", { "class": "powertools-portlet-menu-label" }).append($("<img>", { src: "https://upload.wikimedia.org/wikipedia/commons/1/14/Codex_icon_edit_color-placeholder.svg" }), $("<span>").text(POWER_EDITOR_TOOLBOX_PORTLET_HEADING_TEXT).append($("<span>", { "class": "powertools-portlet-arrow-icon" }).append($("<svg width=\"14\" height=\"14\" viewBox=\"0 0 14 14\"><path d=\"M 2 5 L 12 5 L 7 10 Z\"></path></svg>"))), $("<div>", { "class": "crosshair" }).append($("<svg viewBox=\"0 0 24 24\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\"><g id=\"SVGRepo_bgCarrier\" stroke-width=\"0\"></g><g id=\"SVGRepo_tracerCarrier\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></g><g id=\"SVGRepo_iconCarrier\"> <path d=\"M12 5L12 19\" stroke=\"#200E32\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> <path d=\"M9 17L12 20L15 17\" stroke=\"#200E32\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> <path d=\"M15 7L12 4L9 7\" stroke=\"#200E32\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> <path d=\"M5 12L19 12\" stroke=\"#200E32\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> <path d=\"M17 15L20 12L17 9\" stroke=\"#200E32\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> <path d=\"M7 9L4 12L7 15\" stroke=\"#200E32\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"></path> </g></svg>"))), $("<div>", { "class": "powertools-portlet-body" }).append($("<ul>"))));
	...
	*/
	function addPortletLinkToPowerEditorToolbox(skin, config) {
		...
		if (onClick) if (typeof onClick === "function") portletLink.on("click", onClick);
		else console.error("[PowertoolsPlacement] onClick is not a function");
	...
	*/
	function loadDraggableModule() {
		return mw.loader.getScript("https://cdn.jsdelivr.net/npm/interactjs@1.10.27/dist/interact.min.js");
	}
	/**
	...
	function makePortletDraggable(portlet) {
		loadDraggableModule().done(function() {
			...
			interact(portlet[0]).draggable({
				...
				listeners: {
					move: function(event) {
						...
						setPositionOfDraggableModule(target, (parseFloat(target.getAttribute("data-tx")) || 0) + event.dx, (parseFloat(target.getAttribute("data-ty")) || 0) + event.dy);
					},
					end: function(event) {
						var target = event.target;
						target.classList.remove("dragging");
						saveDraggablePortletOffsetPosition(+target.getAttribute("data-tx"), +target.getAttribute("data-ty"));
			...
			window.addEventListener("resize", mw.util.debounce(function() {
				...
				saveDraggablePortletOffsetPosition(offsetPosition.tx, offsetPosition.ty);
			}, 250));
		}).fail(function(err) {
