//#region gadgets/FandoomUiUtilsUijs/code.js
/**
* Name:        UI-js
...
(function() {
	...
	*/
	function each(obj, cb) {
		if (typeof obj === "object" && typeof cb === "function") for (var i in obj) cb(i, obj[i]);
	}
	/**
	...
	*/
	function createNode(type) {
		switch (type) {
			...
			case void 0:
			case "#document-fragment": return document.createDocumentFragment();
			case "svg":
			case "use":
			case "g":
			case "path":
			case "circle": return document.createElementNS("http://www.w3.org/2000/svg", type);
	...
	*/
	function appendNode(node, parent) {
		if (parent instanceof Node) parent.appendChild(node);
			...
			for (var i = 0; i < elements.length; i++) elements[i].appendChild(node);
		}
	}
	/**
	...
	*/
	function main(opt, parent) {
		if (typeof opt === "string") opt = {
			...
			return opt;
		} else if (typeof opt !== "object") throw new Error("Options parameter incorrect!");
		if ("condition" in opt && !opt.condition) return;
		var el = createNode(opt.type);
		if (typeof opt.text === "string") el.textContent = opt.text;
		if (typeof opt.html === "string") el.innerHTML = opt.html;
		each(opt.attr, function(k, v) {
			if (typeof k === "string") if (nsAttributes[k]) el.setAttributeNS(nsAttributes[k], k.split(":")[1], v);
		...
		if (opt.classes instanceof Array) opt.classes.forEach(function(cls) {
			el.classList.add(cls);
		});
		if (typeof opt.checked === "boolean") el.checked = opt.checked;
		if (typeof opt.selected === "number" && !isNaN(opt.selected)) el.selectedIndex = opt.selected;
		if (typeof opt.value === "number" || typeof opt.value === "string") el.value = String(opt.value);
		if (opt.children instanceof Array) opt.children.forEach(function(c) {
			main(c, el);
		});
