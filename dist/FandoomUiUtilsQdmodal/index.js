//#region gadgets/FandoomUiUtilsQdmodal/code.js
(function($, mw) {
	"use strict";
	var version = 20230117;
	if (mw.libs.QDmodal && mw.libs.QDmodal.version >= version) return;
	var closeText = {
		en: "Close",
		ar: "أغلق",
		be: "Закрыць",
	...
	var $closeIcon = $("<svg class='qdmodal-close' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'><title>" + closeText + "</title><path d='M 3,3 13,13 M 13,3 3,13'/></svg>");
	...
	function addButton(button) {
		var $button = $("<span>");
		if (!button) return;
		if (typeof button.condition === "function" && !button.condition(this)) return;
		if (button.href) $button = $("<a>").attr({
			href: button.href,
			target: "_blank"
		});
		if (typeof button.handler === "function") $button.on("click", button.handler);
		if (button.attr) $button.attr(button.attr);
		$button.addClass("qdmodal-button").text(button.text);
		this.$footer.append($button);
	}
	function closeOnBackgroundClick(event) {
		if (event.target === event.delegateTarget) this.hide();
	}
	function closeOnEscapeKeydown(event) {
		if (event.key === "Escape" && isElementOrChildFrontmost(this.$container[0])) this.hide();
	}
	mw.libs.QDmodal = function(id) {
		if (this === mw.libs) throw new Error("mw.libs.QDmodal should be called as a constructor.");
	...
	mw.libs.QDmodal.prototype.hide = function() {
		if (this.data && typeof this.data.onHide === "function") {
			if (this.data.onHide(this) === false) return;
		}
		this.visible = false;
		this.data = null;
		this.$container.detach();
		if (!visibleModals.length) $body.removeClass("qdmodal-is-visible");
	};
	mw.libs.QDmodal.prototype.show = function(data) {
		if (!data) return;
		this.data = data;
		if (data.title) this.$title.text(data.title);
		if (data.loading) this.$content.empty().append($spinner.clone());
		else this.$content.html(data.content || "");
		this.$footer.empty();
		if (Array.isArray(data.buttons)) data.buttons.forEach(addButton.bind(this));
		if (typeof this.data.onBeforeShow === "function") this.data.onBeforeShow(this);
		if (data.hook) mw.hook(data.hook).fire(this);
		if (!this.visible) {
			$body.addClass("qdmodal-is-visible").append(this.$container);
			this.visible = true;
		}
		if (typeof this.data.onShow === "function") this.data.onShow(this);
	};
