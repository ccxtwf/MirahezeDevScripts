//#region gadgets/MassCopyUpload/code.js
(function(mw) {
	"use strict";
	const TIME_INTERVAL_PER_UPLOAD_IN_MILLISECONDS = (window.MassCopyUpload || {}).TIME_INTERVAL_PER_UPLOAD_IN_MILLISECONDS || 1e3;
	...
	mw.messages.set({
		...
		"ui--overview-explanation": "This is a tool used for transferring images from one wiki to another by copy-uploading the files through URL in batches. To use this tool, <code><nowiki>$</nowiki>wgAllowCopyUploads</code> must be set to true on LocalSettings.php, and the user right <code>upload_by_url</code> must be granted to the user. For more information, refer to the page [[mw:Manual:Configuring_file_uploads/en#Uploading_directly_from_a_URL_(\"sideloading\")|Configuring file uploads]] on the official MediaWiki documentation.",
	...
	let userInputsStore, licensesStore, errorMessagesStore, imagesMetadataStore, uploadTrackingStore;
	function loadInitialState(Vue) {
		userInputsStore = Vue.reactive({
			...
			setFilesToImport(textAreaInput) {
				let arr = textAreaInput.trim().split(/\n+/).map((item) => item.trim()).filter((item) => item !== "").filter((item) => item.match(/^[Ff]ile:/));
				this.usesUserSuppliedUrls = arr.length > 0 && arr.some((item) => item.includes("|"));
				if (this.usesUserSuppliedUrls) arr = arr.filter((item) => item.includes("|"));
	...
	function loadApp({ Vue, Codex, wgSiteName }) {
      ...
      `,
			components: { CdxProgressIndicator: Codex.CdxProgressIndicator },
			...
			computed: {
				initErrorHeading() {
					switch (this.errorMessagesStore.initErrorCode) {
						...
						case 0: return mw.message("error-init--forbidden-user").text();
						case -1: return mw.message("error-init--unexpected").text();
					}
				},
				initErrorDetailsMessageName() {
					switch (this.errorMessagesStore.initErrorCode) {
						case null:
						case false: return null;
						case 0: return "error-init--forbidden-user-more-details";
						case -1: return null;
					}
				}
			}
      ...
      `,
			components: { CdxTextInput: Codex.CdxTextInput },
			setup: () => ({
				userInputsStore,
				errorMessagesStore
			}),
      ...
      `,
			components: { CdxSelect: Codex.CdxSelect },
			computed: { dropdownDefaultLabel() {
      ...
      `,
			components: { CdxTextArea: Codex.CdxTextArea },
			setup: () => ({
				userInputsStore,
				errorMessagesStore,
				limit: MAX_FILE_UPLOADS_PER_BATCH_OPERATION
			}),
			computed: { exceededFileLimit() {
      ...
      `,
			...
			computed: {
				...
				data() {
					return imagesMetadataStore.imagesMetadata.filter(function(item) {
			...
			methods: { toggleFilterUnsuccessful() {
		...
		fetchLicenses();
		checkIfTargetWikiAllowsUploadSideloading().then(() => {
			errorMessagesStore.initErrorCode = null;
		}).catch((code) => {
			errorMessagesStore.initErrorCode = code;
		});
	}
	function onClickedFetchMetadata() {
		...
		if (!window.confirm(mw.message(imagesMetadataStore.imagesMetadata.length === 0 ? "ui--load-image-metadata-list-confirmation" : "ui--clear-image-metadata-list-confirmation").text())) return;
		checkSourceWikiApi(true).then(function(isValidWikiPath) {
			if (!isValidWikiPath) return;
			fetchImageMetadata();
		});
	}
	function beforeUploadFiles({ fromMetadata = false, isReupload = false }) {
		const n = fromMetadata ? imagesMetadataStore.staticUrls().length : userInputsStore.numberOfFilesToImport();
		if (n === 0) {
			mw.notify(mw.message(fromMetadata && !isReupload ? "error-resp--no-metadata-is-fetched" : "error-resp--no-image-to-upload").text());
			return;
		}
	...
	function fetchImageMetadata() {
		...
		const batches = [];
		for (let i = 0; i < Math.ceil(userInputsStore.filesToImport.length / NUMBER_OF_FILES_PER_API_QUERY); i++) batches.push(userInputsStore.filesToImport.slice(i * NUMBER_OF_FILES_PER_API_QUERY, (i + 1) * NUMBER_OF_FILES_PER_API_QUERY));
		...
		const promises = batches.map(function(batch, batchIndex) {
			return new Promise(function(resolve, reject) {
				sourceWikiApi.get({
					...
					resolve(Object.values(res.query.pages).map(function(v, itemIndex) {
						const title = v.title;
						if (v.missing !== void 0) return {
						...
						return {
							...
							pageContents: (v.revisions || []).length === 0 ? null : ((v.revisions[0].slots || {}).main || {})["*"] || null,
		...
		Promise.allSettled(promises).then(function(arrResults) {
			const listData = [];
	...
	function uploadFiles() {
		...
		uploadTrackingStore.uploadProcessId = setInterval(function() {
			...
			while (item !== void 0 && (!item.staticUrl || item.uploadStatusCss === "success")) item = imagesMetadataStore.imagesMetadata[uploadTrackingStore.uploadCounter++];
			if (item === void 0) {
				if (uploadTrackingStore.uploadProcessId) {
			...
			userInputsStore.targetWikiApi().postWithEditToken({
				...
				filename: item.title,
				url: item.staticUrl,
				text: userInputsStore.importPageContents ? item.pageContents || void 0 : (licensesStore.selected || "") === "" ? void 0 : "== {{safesubst:MediaWiki:License-header}} ==\n{{" + licensesStore.selected + "}}",
				ignorewarnings: userInputsStore.ignoreApiWarnings ? 1 : void 0
			}).done(handleUploadResponseFromApi(imagesMetadataStore, item.index)).fail(handleErrorUploadResponseFromApi(imagesMetadataStore, item.index)).always(function() {
				uploadTrackingStore.availableUploadRequestCount++;
			});
	...
	function checkIfTargetWikiAllowsUploadSideloading() {
		return new Promise(function(resolve, reject) {
			userInputsStore.targetWikiApi().getUserInfo().done(function(res) {
				if (res.rights.indexOf("upload_by_url") < 0) reject(0);
				else resolve(true);
			}).fail(function() {
				reject(-1);
			});
	...
	function checkSourceWikiApi(showNotifs) {
		return new Promise(function(resolve) {
			if (userInputsStore.sourceWikiBasicDomain === "") {
				...
				resolve(false);
				if (showNotifs) mw.notify(mw.message("invalid--no-source-wiki-error-details", userInputsStore.sourceWikiApiPath()).text(), { type: "error" });
				return;
			}
			errorMessagesStore.sourceWikiLoadingMessage = mw.message("ui-notif--query-source-wiki", userInputsStore.sourceWikiApiPath()).text();
			errorMessagesStore.sourceWikiErrorMessage = "";
			userInputsStore.sourceWikiApi().getUserInfo().done(function() {
				...
				resolve(false);
				if (showNotifs) mw.notify(mw.message("invalid--cannot-reach-source-wiki", userInputsStore.sourceWikiApiPath()).text(), { type: "error" });
			});
		});
	}
	...
	function fetchLicenses() {
		let options;
		$.get(mw.util.getUrl("MediaWiki:Licenses", { action: "raw" })).done(function(wikitext) {
			...
			options.unshift({
				label: mw.message("ui--license-dropdown-no-license").text(),
				data: null
			});
	...
	function parseWikiLicenses(s, level) {
		...
		for (let i = 0; i < ms.length; i++) {
			...
			let head = parseOneLicenseItem(mt[1], level), inner = mt[2] || "";
			if (inner === "") {
				res.push(head);
				continue;
			}
			...
			for (let j = 0; j < inner.length; j++) res.push(inner[j]);
		}
		return res;
	}
	function translateMwUploadWarnings(apiWarnings) {
		const warningMessages = [];
		if (apiWarnings.exists !== void 0) if ((apiWarnings.exists || {})["no-change"] !== void 0) warningMessages.push(mw.message("upload-response--exists__no-change").text());
		else if ((apiWarnings.exists || {})["duplicateversions"] !== void 0) warningMessages.push(mw.message("upload-response--exists__duplicateversions").text());
		else warningMessages.push(mw.message("upload-response--exists").text());
		[
			"page-exists",
			"was-deleted",
			"duplicate",
			...
			if (apiWarnings[k] !== void 0) warningMessages.push(mw.message("upload-response--" + k).text());
		});
	...
	mw.loader.using(["vue", "@wikimedia/codex"]).then(function(require) {
		loadApp({
			...
			wgSiteName: mwConfig.wgSiteName
		});
