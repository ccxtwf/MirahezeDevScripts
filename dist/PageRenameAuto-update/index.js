//#region gadgets/PageRenameAuto-update/code.js
/**
...
(function() {
	...
	var PRA = {
		...
		updateStatus: function(gifDisplay, message) {
			...
			if (typeof gifDisplay === "string") newMessage = gifDisplay;
			else if (typeof gifDisplay === "boolean") $("#liveLoader").css("display", gifDisplay ? "inline-block" : "none");
			else return false;
			if (typeof newMessage === "string") $("#PRAStatus").html(" " + newMessage);
			return true;
		},
		start: function(callback) {
			if (PRA.started) return false;
			PRA.started = true;
			...
			PRA.api.get({
				...
				if (result.query.pages[-1]) return PRA.getUsage(PRA.oldName);
				PRA.started = false;
				PRA.updateStatus(false, PRA.i18n.msg("destInUse").escape());
				PRA.toggleInputs(true);
				if (typeof callback === "function") callback(false, "destinuse");
			}).then(function(result) {
				if (!result) return;
				...
				if (pageUsage.length > 0) {
					...
					PRA.toggleInputs(true);
					if (typeof callback === "function") callback(false, "filenotused");
				}
			});
		},
		...
		processPageContent: function(callback) {
			for (var i = 0; i < PRA.queueData.length; i++) PRA.requestCompleted[i] = false;
			var promises = [];
			console.log("Getting page contents");
			for (var j = 0; j < Math.floor(PRA.queueData.length / 500) + 1; j++) {
				var tempArray = [];
				for (var k = j * 500; k < j * 500 + 500 && k < PRA.queueData.length; k++) tempArray.push(PRA.queueData[k]);
				if (tempArray.length === 0) break;
				while (tempArray.length > 0) promises.push(PRA.api.post({
					action: "query",
					prop: "revisions",
					rvprop: "content",
			...
			$.when.apply(this, promises).then(function() {
				...
				if (escapedName.slice(0, 1).match(/[A-Za-z]/i)) escapedName = "[" + escapedName.slice(0, 1).toUpperCase() + escapedName.slice(0, 1).toLowerCase() + "]" + escapedName.slice(1);
				var escapedFileNamespaceNames = Object.entries(PRA.wg.wgNamespaceIds).flatMap(function(entry) {
					...
					escapedFileNamespaceName = "[" + escapedFileNamespaceName.slice(0, 1).toUpperCase() + escapedFileNamespaceName.slice(0, 1).toLowerCase() + "]" + escapedFileNamespaceName.slice(1);
					return [escapedFileNamespaceName];
				});
				var isFile = false;
				if (escapedFileNamespaceNames.some(function(escapedFileNamespaceName) {
					return escapedName.startsWith(escapedFileNamespaceName);
				})) {
					escapedName = "(" + escapedFileNamespaceNames.join("|") + ")?" + escapedName.slice(escapedName.indexOf(":") + 1);
					isFile = true;
					PRA.newNameWithoutNamespace = PRA.newName.slice(PRA.newName.indexOf(":") + 1);
				}
				var pageReplacement = new RegExp("(:?|=[ ]*?|\\||\\[|\\{)\\s*" + escapedName + "\\s*(.*?\\n|[ ]*?\\||\\]|\\})", "g");
				var replacementReg = new RegExp(escapedName, "g");
				PRA.pageData.forEach(function(data) {
					if (data.content.search(pageReplacement) === -1) PRA.failedLog(data.title);
					else {
						data.changed = true;
				...
				$(".PRAinfo").append(ui.div({
					attrs: { id: "PRAQueueProgress" },
					child: ui.div({ attrs: { id: "PRAProgressInd" } })
				}));
				PRA.queueProgress = 0;
				var l = 0;
				var throttle = setInterval(function() {
					if (PRA.pageData[l].changed === true) PRA.submitChangedPages(l, callback);
					else {
						PRA.updateQueueProgress();
						PRA.requestCompleted[l] = true;
					}
					l++;
					if (l === PRA.pageData.length) clearInterval(throttle);
		...
		failedLog: function(page) {
			var ui = window.dev.dorui;
			$("#PRAFailedLog").append(ui.div({ child: ui.a({
				attrs: {
		...
		updateQueueListing: function() {
			...
			if (!PRA.queueData || PRA.queueData.length < 1) {
				$("#PRAQueue").html(ui.div({ text: PRA.i18n.msg("nothingInQueue").plain() }));
		...
		createActionButton: function(id, event, options) {
			...
			if (options.style) actionButton.css(options.style);
		...
		setupForm: function() {
			...
			$("#bodyContent").html(ui.frag([
				ui.p({ html: i18n.msg("infoText").parse() }),
				ui.p({ html: i18n.msg("notMovedNote").parse() }),
				ui.p({ html: i18n.msg("warning").parse() }),
				ui.fieldset({ children: [ui.legend({ text: i18n.msg("fieldTitle").plain() }), ui.table({
					attrs: { id: "mw-renamepage-table" },
					children: [
						PRA.labelAndInput(ui, "currentName", ui.a({
							attrs: { href: mw.util.getUrl(page) },
							text: page
						})),
						PRA.labelAndInput(ui, "newNameField", ui.input({ attrs: {
							name: "wpNewTitleMain",
							value: page,
							type: "text",
							id: "wpNewTitleMain",
						...
						PRA.labelAndInput(ui, "reason", ui.textarea({ attrs: {
							name: "wpReason",
							id: "wpReason",
							cols: 60,
							rows: 2,
						...
						ui.tr({ children: [ui.td({ html: "&#160;" }), ui.td({
							classes: ["mw-submit"],
							children: [
								this.createActionButton("PRAstart", PRA.start, {
									label: i18n.msg("populateListButton").plain(),
									flags: ["progressive", "primary"]
								}),
								this.createActionButton("PRAprocess", PRA.processQueue, {
								...
								ui.span({ attrs: { id: "PRAStatus" } })
							]
						})] }),
						PRA.labelAndInput(ui, "queuedItems", ui.div({ attrs: { id: "PRAQueue" } })),
						ui.tr({ children: [ui.td({
							classes: ["mw-label"],
							html: "&#160;"
						}), ui.td({
							classes: ["mw-input", "PRAinfo"],
							child: ui.div({
								attrs: { id: "PRAQueueLength" },
								html: i18n.msg("pagesInQueue").escape().replace("$1", ui.span({ attrs: { id: "PRAQueueLengthBox" } }).outerHTML)
							})
						})] }),
						PRA.labelAndInput(ui, "failedItems", ui.div({
							attrs: { id: "PRAFailedLog" },
							text: i18n.msg("failedItemsInfo").plain()
						}))
					]
				})] })
			]));
			$("#liveLoader").append($.createSpinner({
				size: "small",
				type: "inline"
			}));
		...
		initialize: function(i18nLoader) {
			...
			if (PRA.wg.wgCanonicalSpecialPageName === "Blankpage" && mw.util.getParamValue("blankspecial") === "pageusageupdate") {
				if (mw.loader.getState(".FandoomUiUtilsDorui") === null) importScriptURI("https://cdn.jsdelivr.net/gh/ccxtwf/MirahezeDevScripts@1776596079/dist/FandoomUiUtilsDorui/gadget-impl.js");
				mw.hook("dev.doru.ui").add(this.setupForm.bind(this));
				return;
			}
			if (mw.loader.getState("ext.gadget.store.PowertoolsPlacement") !== null) mw.hook("dev.powertools.placement").add(function(placement) {
	...
	function getI18nLoader() {
		...
		function onLoadedModule(module) {
			return module.loadMessages("PageRenameAuto-update", {});
		}
	...
	function getFallbackMessages() {
		...
		msgMap.set({
			"buttonText": "Rename & update",
			"summary": "Updating page links (automatic)",
			"header": "Renaming page: $1",
			"infoText": "Using the form below will rename a page and change the page names on pages that the page is used on. Be sure to check [[Special:WantedCategories|wanted categories]]. You are responsible for making sure that links continue to point where they are supposed to go.",
			"notMovedNote": "Note that the page will <strong>not</strong> be moved if there is already a page at the new title.",
			"warning": "<strong>Warning!</strong> This can be drastic and unexpected for a popular page; please be sure you understand the consequences of this before proceeding.",
			"fieldTitle": "Rename page & update usage",
			"currentName": "Current name:",
			"newNameField": "New name:",
			"reason": "Reason:",
			"populateListButton": "Populate list",
			"queuedItems": "Queued items:",
			"nothingInQueue": "There is currently nothing in the queue.",
			"pagesInQueue": "Pages in queue: $1",
			"failedItems": "Failed items:",
			"failedItemsInfo": "Failed items appear here after execution. Note that pages that the page is transcluded through a template on will also appear here falsely.",
			"processing": "Processing...",
			"successful": "Successful!",
			"processQueueButton": "Process queue",
			"linkNewPage": "Link",
			"notSubmitted": "The page \"$1\" could not be submitted because of error code: $2. Please update the link(s) on that page manually.",
			"articleExistsPrompt": "The page \"$1\" was unable to be moved to \"$2\" for reason: $3.",
			"anotherDestName": "Please enter another destination name for this page, or cancel the moving.",
			"notUsed": "Page not linked on any pages.",
			"destInUse": "Destination name already in use.",
			"title": "Page Rename Auto-Update"
		});
