//#region gadgets/FandoomUiUtilsModal/code.js
/**
* Name:        Modal
* Version:     v2.2
...
(function() {
	...
	*/
	function init() {
		module._windowManager = new OO.ui.WindowManager({ classes: ["modal-js-window"] });
	...
	*/
	function ModalButton(options) {
		...
		this.type = "button";
		this.setText(options.text || options.value).setEvent(options.event).setClasses(options.classes).setID(options.id).setDisabled(options.disabled).setSprite(options.sprite || options.imageClass).setHref(options.href).setTitle(options.title).setTarget(options.target).setName(options.name);
	}
	/**
	...
	*/
	ModalButton.prototype.setEvent = function(event) {
		if (typeof event === "string") this.event = event;
		return this;
	};
	/**
	...
	*/
	ModalButton.prototype.setHref = function(href) {
		if (this.type === "link" && typeof href === "string") this.href = href;
		return this;
	};
	/**
	* Sets the button ID.
	...
	*/
	ModalButton.prototype.setName = function(name) {
		if (this.type === "input") if (typeof name === "string") this.name = name;
		else throw new Error("`name` parameter required!");
		return this;
	};
	/**
	* Sets button text.
	...
	*/
	ModalButton.prototype.setText = function(text) {
		if (typeof text !== "string") throw new Error("No text specified!");
		this.text = text;
		return this;
	};
	/**
	...
	*/
	ModalButton.prototype.setSprite = function(sprite) {
		if ((this.type === "link" || this.type === "button") && typeof sprite === "string") this.sprite = sprite;
		return this;
	};
	/**
	...
	*/
	ModalButton.prototype.setTarget = function(target) {
		if (this.type === "link" && typeof target === "string") this.target = target;
		return this;
	};
	/**
	...
	*/
	ModalButton.prototype.setTitle = function(title) {
		if (this.type === "link" && typeof title === "string") this.title = title;
		return this;
	};
	/**
	...
	*/
	function Modal(options) {
		if (typeof options.id !== "string") throw new Error("Modal ID must be specified!");
		if (module.modals[options.id]) throw new Error("Modal with same ID already registered!");
		this.id = options.id;
		this.context = options.context || this;
		this.setSize(options.size).setContent(options.content).setTitle(options.title, options.isHTML).setCloseTitle(options.closeTitle).setButtons(options.buttons).setEvents(options.events).setClass(options.class || options.classes).setClose(options.close).setCloseOnClickingBackdrop(options.closeOnClickingBackdrop).setCloseEscape(options.closeEscape);
		module.modals[this.id] = this;
	}
	/**
	* Couples the dependencies oojs, oojs-ui, oojs-ui-windows by late binding 
	*/
	Modal.prototype.deps = { OO: null };
	/**
	...
	*/
	Modal.prototype.setButtons = function(buttons) {
		this.buttons = buttons instanceof Array ? buttons.map(createButton).filter(Boolean) : [];
	...
	*/
	Modal.prototype.setClass = function(classes) {
		if (classes instanceof Array) this.classes = classes;
		else if (typeof classes === "string") this.classes = [classes];
		return this;
	};
	/**
	* Sets the modal's class(es).
	...
	*/
	Modal.prototype.setClose = function(close) {
		if (typeof close === "function") this.closeFunc = close;
		return this;
	};
	/**
	...
	*/
	Modal.prototype.setContent = function(content) {
		if (typeof content === "string" || typeof content === "object" && content instanceof Modal.prototype.deps.OO.ui.Layout) this.content = content;
		else if (content instanceof Node) this.content = content;
		else if (typeof content === "object" && typeof window.dev.ui === "function") this.content = window.dev.ui(content);
		else throw new Error("Modal content not specified!");
		if (this._modal) if (this.content instanceof Modal.prototype.deps.OO.ui.Layout) {
			this._modal.content.$element.remove();
			this._modal.content = this.content;
			this._modal.$body.append(this.content.$element);
		} else this._modal.content.$element.html(this.content);
		return this;
	};
	/**
	...
	*/
	Modal.prototype.setEvent = function(name, listener) {
		...
		if (typeof listener === "function") this.events[name].push(listener.bind(this.context));
		else if (typeof listener === "string" && this.context && typeof this.context[listener] === "function") this.events[name].push(this.context[listener].bind(this.context));
		return this;
	};
	/**
	* Sets event handlers.
	...
	*/
	Modal.prototype.setEvents = function(events) {
		...
		for (var e in events) if (events[e] instanceof Array) for (var i = 0, l = e.length; i < l; ++i) this.setEvent(e, events[e][i]);
		else this.setEvent(e, events[e]);
		return this;
	};
	/**
	* Sets the modal size.
	...
	*/
	Modal.prototype.setSize = function(size) {
		if (MODAL_SIZES.indexOf(size) === -1) this.size = "medium";
		else if (size === "content-size") this.size = "full";
		else this.size = size;
		if (this._modal) this._modal.setSize(this.size);
		return this;
	};
	/**
	...
	*/
	Modal.prototype.setTitle = function(title, isHTML) {
		this.title = typeof title === "string" ? title : "Modal";
	...
	*/
	Modal.prototype.filterActionButtons = function(filters) {
		var o = void 0, ids = [];
		if (typeof filters === "string") ids = [filters];
		else if (filters instanceof Array) ids = filters;
		else if (typeof filters === "object") {
			if (filters.actions !== void 0 || filters.flags !== void 0) o = {
	...
	*/
	Modal.prototype.create = function() {
		...
		Modal.prototype.deps.OO.inheritClass(OOUIModal, Modal.prototype.deps.OO.ui.ProcessDialog);
		...
		OOUIModal.prototype.close = function() {
			...
			if (this._modal.closeFunc) shouldClose = this._modal.closeFunc.bind(this._modal.context)() !== false;
			if (shouldClose) _close.call(this);
		};
		OOUIModal.prototype.initialize = function() {
			superclass.initialize.apply(this, arguments);
			if (this._modal.content instanceof Modal.prototype.deps.OO.ui.Layout) this.content = this._modal.content;
			else this.content = new Modal.prototype.deps.OO.ui.PanelLayout({
				expanded: false,
				padded: false
			});
		...
		OOUIModal.prototype.getActionProcess = function(action) {
			...
			if (action === "close") return new Modal.prototype.deps.OO.ui.Process((function() {
				this.close();
			}).bind(this));
			if (this._modal.events[action]) return new Modal.prototype.deps.OO.ui.Process((function() {
		...
		this._modal.$frame.prev().click((function(event) {
			if (this.closeOnClickingBackdrop && $(event.target).parent().attr("id") === this.id) this._modal.close();
		}).bind(this));
	...
	*/
	Modal.prototype.show = function() {
		...
		if (this._modal) module._windowManager.openWindow(this._modal);
		else if (this._loading) this._loading.then((function() {
			this._modal.show();
		}).bind(this));
		else throw new Error("Modal not created!");
	};
	/**
	* Closes the modal
	*/
